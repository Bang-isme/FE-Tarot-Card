...
export const createAdminAccount = async (userData) => {
  ...
  // Real API
  try {
    const response = await axiosInstance.post('/api/admin/users/create-admin', userData);
    return response.data;
  } catch (error) {
...
export const exchangeGoogleCode = async (code) => {
  try {
    ...
    const response = await axiosInstance.post('/api/auth/social/google/exchange', { code });
...
export const socialLogin = async (provider, authResult) => {
  try {
    ...
    const response = await axiosInstance.post(`/api/auth/social/${provider.toLowerCase()}`, {
      token: authResult.token // Đây là access_token của Facebook
    });
    ...
    throw error.response?.data || { message: `Lỗi đăng nhập qua ${provider}` };
  }
};
