...
// Tách component error message
const ErrorMessage = memo(({ errors, delay }) => {
...
const InputField = memo(({ id, label, type, value, onChange, placeholder, rightLink, delay, error }) => {
    ...
    <div className={`transition-all duration-500 ${delay ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: delay }}>
      ...
      <div className="relative">
        <input
          ...
          } text-white focus:outline-none focus:ring-2 focus:ring-[#9370db]/50 focus:border-[#9370db] transition-all placeholder:text-gray-500`}
      ...
      <ErrorMessage errors={error} delay={0.1} />
    </div>
  );
});
...
const RegisterForm = ({ onClose, onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) {
      ...
      errors.name = ['Họ tên phải có ít nhất 4 ký tự'];
      isValid = false;
    } else if (name.length > 50) {
      errors.name = ['Họ tên không được vượt quá 50 ký tự'];
      isValid = false;
    } else if (!/^[A-ZĐÁÀẢÃẠÂẤẦẨẪẬĂẮẰẲẴẶÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴ][a-zđáàảãạâấầẩẫậăắằẳẵặéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵ]*(?:[ ][A-ZĐÁÀẢÃẠÂẤẦẨẪẬĂẮẰẲẴẶÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴ][a-zđáàảãạâấầẩẫậăắằẳẵặéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵ]*)+$/.test(name)) {
      errors.name = ['Họ tên phải viết hoa chữ cái đầu và chỉ chứa chữ cái tiếng Việt'];
      isValid = false;
    }
    ...
    if (!birthDate) {
      ...
      if (birthDateObj > today) {
        errors.birthDate = ['Ngày sinh không thể là ngày trong tương lai'];
        isValid = false;
      } else if (birthDateObj > minAge) {
        errors.birthDate = ['Bạn phải đủ 13 tuổi trở lên'];
        isValid = false;
      } else if (birthDateObj < maxAge) {
    ...
    if (!email) {
      ...
      const blacklistedDomains = [
        ...
        '10minutemail.com', 'trash-mail.com', 'fakeinbox.com'
      ];
      if (blacklistedDomains.includes(domain)) {
        errors.email = ['Email tạm thời không được chấp nhận'];
        isValid = false;
      }
    }
    ...
    if (!phone) {
      ...
      const validPrefixes = {
        '86': 'Viettel', '96': 'Viettel', '97': 'Viettel', '98': 'Viettel',
        '32': 'Viettel', '33': 'Viettel', '34': 'Viettel', '35': 'Viettel',
        '36': 'Viettel', '37': 'Viettel', '38': 'Viettel', '39': 'Viettel',
        '89': 'Mobifone', '90': 'Mobifone', '93': 'Mobifone', '70': 'Mobifone',
        '76': 'Mobifone', '77': 'Mobifone', '78': 'Mobifone', '79': 'Mobifone',
        '88': 'Vinaphone', '91': 'Vinaphone', '94': 'Vinaphone', '83': 'Vinaphone',
        '84': 'Vinaphone', '85': 'Vinaphone', '81': 'Vinaphone', '82': 'Vinaphone',
        '92': 'Vietnamobile', '56': 'Vietnamobile', '58': 'Vietnamobile',
        '99': 'Gmobile', '59': 'Gmobile'
      };
      if (!validPrefixes[mobilePrefix]) {
        errors.phone = ['Đầu số điện thoại không hợp lệ'];
        isValid = false;
      }
    }
    ...
    if (!password) {
      ...
      if (password.length > 50) {
        passwordErrors.push('Mật khẩu không được vượt quá 50 ký tự');
      }
      if (!/[A-Z]/.test(password)) {
        passwordErrors.push('Mật khẩu phải chứa ít nhất 1 chữ hoa');
      }
      if (!/[a-z]/.test(password)) {
        passwordErrors.push('Mật khẩu phải chứa ít nhất 1 chữ thường');
      }
      if (!/[0-9]/.test(password)) {
        passwordErrors.push('Mật khẩu phải chứa ít nhất 1 số');
      }
      if (!/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
        passwordErrors.push('Mật khẩu phải chứa ít nhất 1 ký tự đặc biệt');
      }
      if (/(.)\1{2,}/.test(password)) {
        passwordErrors.push('Mật khẩu không được chứa 3 ký tự giống nhau liên tiếp');
      }
      if (password.toLowerCase().includes(name.toLowerCase())) {
        passwordErrors.push('Mật khẩu không được chứa tên của bạn');
      }
  ...
  // Chỉ lấy thông báo lỗi đầu tiên cho mỗi trường
  const errorMessages = Object.values(passwordError).map(errors => errors[0]);
          ...
          <div className="relative z-10">
            ...
            {error && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="mb-4 p-3 rounded-lg bg-red-500/20 border border-red-500/30 text-white text-sm"
              >
                {error}
              </motion.div>
            )}
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className={`transition-all duration-500 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: isLoaded ? '200ms' : '0ms' }}>
                <label className="block text-sm font-medium text-gray-300 ml-1 mb-2 tracking-vn-tight">
                  Giới tính
                </label>
                <div className="grid grid-cols-3 gap-3">
                  <label className="relative flex cursor-pointer rounded-lg bg-white/5 border border-white/10 p-2 shadow-sm hover:border-[#9370db]/30 transition-colors">
                    <input
                      type="radio"
                      name="gender"
                      value="male"
                    ...
                    <span className="flex flex-1 items-center justify-center">
                      <span className="flex items-center">
                        <span className={`h-4 w-4 rounded-full border ${gender === 'male' ? 'border-[#9370db] bg-[#9370db]' : 'border-white/30'} mr-2 flex items-center justify-center`}>
                          {gender === 'male' && (
                            <span className="h-2 w-2 rounded-full bg-white" />
                          )}
                        </span>
                        <span className="text-sm font-medium text-gray-300">Nam</span>
                      </span>
                    </span>
                  </label>
                  <label className="relative flex cursor-pointer rounded-lg bg-white/5 border border-white/10 p-2 shadow-sm hover:border-[#9370db]/30 transition-colors">
                    ...
                    <span className="flex flex-1 items-center justify-center">
                      <span className="flex items-center">
                        <span className={`h-4 w-4 rounded-full border ${gender === 'female' ? 'border-[#9370db] bg-[#9370db]' : 'border-white/30'} mr-2 flex items-center justify-center`}>
                          {gender === 'female' && (
                            <span className="h-2 w-2 rounded-full bg-white" />
                          )}
                        </span>
                        <span className="text-sm font-medium text-gray-300">Nữ</span>
                      </span>
                    </span>
                  </label>
                  <label className="relative flex cursor-pointer rounded-lg bg-white/5 border border-white/10 p-2 shadow-sm hover:border-[#9370db]/30 transition-colors">
                    ...
                    <span className="flex flex-1 items-center justify-center">
                      <span className="flex items-center">
                        <span className={`h-4 w-4 rounded-full border ${gender === 'other' ? 'border-[#9370db] bg-[#9370db]' : 'border-white/30'} mr-2 flex items-center justify-center`}>
                          {gender === 'other' && (
                            <span className="h-2 w-2 rounded-full bg-white" />
                          )}
                        </span>
                        <span className="text-sm font-medium text-gray-300">Khác</span>
                      </span>
                    </span>
                  </label>
                </div>
                <ErrorMessage errors={errorMessages} delay={0.1} />
              </div>
              ...
              <div className={`transition-all duration-500 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: '800ms' }}>
                <div className="flex items-center">
                  <input
                    ...
                    required
                  />
                  <label htmlFor="terms" className="ml-2 block text-sm text-gray-300 tracking-vn-tight">
                    Tôi đồng ý với{" "}
            ...
            <div className={`mt-6 text-center transition-all duration-500 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: '1000ms' }}>
            ...
            <div className={`mt-6 grid grid-cols-2 gap-3 transition-all duration-500 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: '1100ms' }}>
